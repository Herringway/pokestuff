...
class gen5 extends basegame {
	...
	function getMoveList($id, $level = -1) {
		...
		if ($level == -1) {
			...
			for ($i = 0; $i < 101; $i++)
				if (substr($str, $i, 1) == '1')
					$moves['TM'][] = array('Learned' => $i > 94 ? 'HM'.($i-94) : 'TM'.($i+1), 'id' => $gamecfg['TM Map'][$i]);
			if (isset($rawdata[56]))
				for ($i = 0; $i < 8; $i++)
	...
	function getTrainerPokemon($id) {
		...
		for ($i = 0; $i < $numpokes; $i++) {
			...
			if ($movelist)
				for ($j = 0; $j < 4; $j++)
					$moves[] = ord($trpoke[$i*$length+$length-8+$j*2])+(ord($trpoke[$i*$length+$length-7+$j*2])<<8);
			else {
	...
	function getArea($id) {
		...
		for ($i = 0; $i < strlen($data)/232; $i++) {
			...
			for ($j = 2; $j < 58; $j++) {
				...
				if (!isset($output['Encounters'][$gamecfg['Area Data']['Labels'][$k].$season][$id]))
					$output['Encounters'][$gamecfg['Area Data']['Labels'][$k].$season][$id] = $tmp;
				else {
					$output['Encounters'][$gamecfg['Area Data']['Labels'][$k].$season][$id]['flags'][] = $tmp['flags'][0];
					$output['Encounters'][$gamecfg['Area Data']['Labels'][$k].$season][$id]['minlevel'] = min($output['Encounters'][$gamecfg['Area Data']['Labels'][$k].$season][$id]['minlevel'], $tmp['minlevel']);
					$output['Encounters'][$gamecfg['Area Data']['Labels'][$k].$season][$id]['maxlevel'] = max($output['Encounters'][$gamecfg['Area Data']['Labels'][$k].$season][$id]['maxlevel'], $tmp['maxlevel']);
				}
			}
		}
