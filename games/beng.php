...
class beng {
	...
	function getNumberPokemon() {
		$this->loadNarc('stats');
		return $this->narcs['stats']->Files-1;
	}
	function getNumberMoves() {
		$this->loadNarc('movedata');
		return $this->narcs['movedata']->Files-1;
	}
	function loadText() {
		if ($this->textdata === null)
			$this->textdata = new gen5text('games/beng/0/0/2');
	}
	function loadStoryText() {
		if ($this->storyTextData === null)
			$this->storyTextData = new gen5text('games/beng/0/0/3');
	}
	function getText() {
		$this->loadText();
		$this->loadStoryText();
	...
	function getName($id) {
		...
		'Meloetta (Step)');
		if ($id > 649)
			return $pknobw[$id-650];
		$this->loadText();
		return $this->textdata[70][$id];
	}
	function getSpecies($id) {
		$this->loadText();
		return $this->textdata[260][$id];
	}
	...
	function getMoveName($id) {
		$this->loadText();
		return $this->textdata[203][$id];
	}
	function getMoveDesc($id) {
		$this->loadText();
		return $this->textdata[202][$id];
	}
	function getAbilityName($id) {
		$this->loadText();
		return $this->textdata[182][$id];
	}
	function getAbilityDesc($id) {
		$this->loadText();
		return $this->textdata[183][$id];
	}
	function getItemName($id) {
		$this->loadText();
		return $this->textdata[54][$id];
	}
	function getItemDesc($id) {
		$this->loadText();
		return $this->textdata[53][$id];
	}
	...
	function getMove($id) {
		...
		$output['id'] = $id;
		$output['desc'] = $this->getMoveDesc($id);
		return $output;
	}
