...
class bjpn {
	...
	static function getNumberPokemon() {
		$narc = new NARCFile('games/bjpn/0/1/6');
		return $narc->Files-1;
	}
	function loadText() {
		if ($this->textdata === null)
			$this->textdata = new gen5text('games/bjpn/0/0/2');
	}
	function loadStoryText() {
		if ($this->storyTextData === null)
			$this->storyTextData = new gen5text('games/bjpn/0/0/3');
	}
	function getText() {
		$this->loadText();
		$this->loadStoryText();
